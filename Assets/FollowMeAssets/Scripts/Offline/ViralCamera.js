#pragma strict
enum FollowMeCamera {Fixed,Orbit,Chase}
var cameraType : FollowMeCamera = FollowMeCamera.Fixed;
...
function LateUpdate (){
	...
	if(cameraType == FollowMeCamera.Fixed){
		if(dDir.x > 0){
	...
	else if(cameraType == FollowMeCamera.Orbit){
    ...
    else if(cameraType == FollowMeCamera.Chase){
    	position = target.TransformPoint(0, height, -distance); 
		myTransform.position = Vector3.Lerp (myTransform.position, position, Time.deltaTime * 2.5); 
	
		rotation = Quaternion.LookRotation(target.position - myTransform.position, target.up);
		myTransform.rotation = Quaternion.Slerp (myTransform.rotation, rotation, Time.deltaTime * 2.5);
    }
}
